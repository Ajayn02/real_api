...
exports.getUserAnalytics = async (req, res) => {
    try {
        const users = await adminService.getUserAnalytics()
        const monthMap = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
        const lastSixMonths = Array.from({ length: 6 }).map((_, i) => {
            const date = new Date();
            date.setMonth(date.getMonth() - (5 - i));
        ...
        const monthWiseUserCounts = lastSixMonths.map(({ key, count }) => ({
            month: key,
            count
        }));

        sendResponse(res, 200, 'user analytics retrived successfully', monthWiseUserCounts)
    } catch (error) {
        sendError(res, 500, 'Failed to retrive posts', error)
    }
}